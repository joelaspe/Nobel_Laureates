...
/*** GLOBAL VARIABLES */
const NOBELPRIZE_API_BASE_URL = 'https://api.nobelprize.org/2.1/nobelPrize/';
const LAUREATE_API_BASE_URL = 'https://api.nobelprize.org/2.1/laureate/';
const WIKIDATA_BASE_API_URL = "https://www.wikidata.org/wiki/Special:EntityData/";
const WIKIPEDIA_BASE_API_URL = 'https://en.wikipedia.org/w/api.php?action=parse&prop=text|images&format=json&origin=*&page=';
...
async function generateLaureates(prizeData) {
    for(let i = 0; i < prizeData[0].laureates.length; i++) {
...
function processData(data) {
    //remove any old spans or blockquotes holding search results
    $('.laureate-container').remove();
    $('.award-quotation').remove();
    
    //generate html elements
    
    $('.result-container').append($('<div/>').addClass('award-quotation'));
    const $awardQuotation = $('.award-quotation');

    // Check if no data was returned (certain years or current year) 
    if(!('laureates' in data[0])) {
        $awardQuotation.append($('<blockquote/>').append($('<p/>')).text("No award was given that year."));    
    } else { //good data is returned, process it and make DOM elements
        generateLaureates(data);
        var laureateNames = []; // holds up to 3 laureate names, used to make the motivation quote
        ...
        for(let i = 0; i < data[0].laureates.length; i++) {
            ...
            if('orgName' in data[0].laureates[i]) {
                laureateNames[i] = data[0].laureates[i].orgName.en
            } else {
              laureateNames[i] = data[0].laureates[i].fullName.en
            }
        }    
        //Wording for joint awards is different, need to check length of the laureteNames array
        if(laureateNames.length > 1) { //FIXME: can I combine this into one line?
            //generate joint citation
...
function generateApiStringCategoryAndYear(category, year) {
    return NOBELPRIZE_API_BASE_URL + category + "/" + year;
}

/** Takes in an array of names and produces a string that separates
 * the names by commas, except the last one which separates with 'and'
 * Max array size is given at 3. It is given that the min size of the array will be 2.
 */
